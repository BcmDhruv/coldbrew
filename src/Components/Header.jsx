    ...
    <header className={`header ${isAboutPage ? "about-header" : ""}`}>
        ...
        >
          <svg viewBox="0 0 32 32">
            <path
              ...
              className={`line line-top-bottom ${isAboutPage ? "about-line" : ""}`}
            ></path>
            
            <path 
              d="M7 16 27 16" 
              className={`line ${isAboutPage ? "about-line" : ""}`} 
            ></path>
          </svg>
      ...
      <nav className={`dropdown ${menuOpen ? "open" : ""}`}>
        <ul>
          ...
          <li><Link to="/#contact"  className="nav-link cta" onClick={() => setMenuOpen(false)}>Book Discovery Call</ Link></li>
        </ul>
      </nav>
    </header>
  );
